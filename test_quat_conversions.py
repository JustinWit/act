...
for i in tqdm(range(50)):
    ...
    with open(pkl_path, 'rb') as f:
        data = pkl.load(f)
    for i in range(data['arm_action'].shape[0]):
        deoxys = quat2axisangle(data['eef_quat'][i])
        scipy = R.from_quat(data['eef_quat'][i]).as_rotvec()
        print()
        print("quat", data['eef_quat'][i])
        print("deoxys axis-angle:", deoxys)
        print("scipy axis-angle:", scipy)
        if not np.allclose(deoxys, scipy):
            breakpoint()
